...
def main() :
    ...
    if choice == menu[0] :
        # accept_multiple_files를 True로 셋팅시
        # 여러 파일들을 한꺼번에 받을 수 있다.
        upload_files = st.file_uploader('이미지 파일들 선택', type = ['jpg', 'png', 'jpeg'], 
        accept_multiple_files = True)

        # upload_files는 여러 파일들을 저장하고 있는 리스트
        # 업로드한 파일이 있는 경우에만, 아래 코드를 실행
        if upload_files is not None :

            # 업로드한 파일이 리스트, 하나씩 가져온 다음
            # temp 폴더에 저장할 것이다.
            for file in upload_files :

                # 파일명을 유니크하게 만들어서 저장해야한다.
                # 현재시간을 활용해서, 파일명을 만든다.
